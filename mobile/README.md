...
### 🤖 데모 모드에서도 "진짜 AI 대화" 켜기 (선택)

키가 없으면 예시 답변(정해진 문구)만 나옵니다. 실제 Claude로 대화하려면:

```bash
cd mobile
cp .env.example .env          # 그리고 .env 를 열어 키를 붙여넣기
npx expo start -c             # -c = 캐시 지우고 시작 (env 반영 필수)
```

- 배너가 **초록색 "실제 AI 대화 연결됨 ✓"** 이면 연결된 것.
- 여전히 예시로 답한다면 답변 카드에 **실패 이유(키 오류/한도 초과/네트워크)** 가 표시됩니다.
  Expo 를 켠 터미널 로그의 `[LLM]` 줄도 함께 확인하세요.
- 키를 바꾼 뒤에는 **반드시 `-c` 로 재시작**(EXPO_PUBLIC_* 는 번들 시점에 박히므로 캐시가 남으면 옛 값이 쓰임).
- `sk-ant-` 로 시작하는 키인지, 앞뒤 따옴표·공백이 없는지 확인하세요.
