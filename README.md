...
## 🚀 가장 쉬운 테스트 (브라우저 = 진짜 Claude 대화)

```bash
./test.sh            # 준비 → 서버 실행 → http://localhost:3000 안내까지 한 번에
```
> 폰(Expo Go)은 키·네트워크 설정이 까다로워요. **브라우저 테스트**는 서버 쪽 키 하나만 있으면
> 클라이언트 키 없이 **실제 Claude 대화**가 바로 됩니다. `.env` 의 `ANTHROPIC_API_KEY` 만 채우면 끝.

테스트 순서: ① '전기요금 아끼는 법' 질문 → 실제 답변  ② '정수기 렌탈 추천해줘' → 혜택 카드
③ 무료 대화 5회 소진 → **광고 보고 이어가기(+20P)** → 계속  ④ 내 보상 탭에서 적립·쿠폰 교환.
